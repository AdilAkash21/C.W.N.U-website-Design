...
export function AdminDepartmentsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        <div className="border-b border-gray-100 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-950/30 sm:p-5">
          <div className="min-w-0 md:max-w-2xl">
            <Input placeholder="Search departments..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} leftIcon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>} />
          </div>
        </div>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {departments.map((d: any) => (
                    <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="p-4 hidden md:table-cell text-sm text-gray-600 dark:text-gray-400">{d.head ? `${d.head.firstName} ${d.head.lastName}` : '—'}</td>
      ...
      <DepartmentFormModal dept={selectedDept} isOpen={!!selectedDept || showCreateModal} onClose={() => { setSelectedDept(null); setShowCreateModal(false); }} onSubmit={selectedDept ? (data: any) => updateDeptMutation.mutate({ id: selectedDept.id, data }) : (data: any) => createDeptMutation.mutate(data)} isLoading={createDeptMutation.isPending || updateDeptMutation.isPending} teachers={teachers} />
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={dept ? 'Edit Department' : 'Create Department'} size="lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        <div className="grid sm:grid-cols-2 gap-5">
          <Input label="Department Code" error={errors.code?.message?.toString()} {...register('code', { required: true })} />
          <Input label="Department Name" error={errors.name?.message?.toString()} {...register('name', { required: true })} />
        </div>
