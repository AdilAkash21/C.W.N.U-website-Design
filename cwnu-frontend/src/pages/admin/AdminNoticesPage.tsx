    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-4">
          ...
          <Select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1); }} options={['All', ...noticeTypes].map(t => ({ value: t, label: t === 'All' ? 'All Types' : t.charAt(0).toUpperCase() + t.slice(1) }))} className="w-40" />
        </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={notice ? 'Edit Notice' : 'Create Notice'} size="lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        ...
        <Textarea label="Content" rows={5} error={errors.content?.message} {...register('content', { required: true })} />
