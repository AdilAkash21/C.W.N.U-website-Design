...
export function HomePage() {
  ...
  const primaryPath = !isAuthenticated ? '/register' : user?.role === 'student' ? '/courses' : dashboardPath;
  const courses = [
    {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#fbf7ed] via-[#f3ead7] to-[#eadcc3] dark:from-academic-navy dark:via-academic-navy-light dark:to-academic-dark" />
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=1920&q=80')] bg-cover bg-center opacity-[0.07] dark:opacity-10" />
        <div className="absolute inset-0 bg-grid-pattern opacity-[0.08] dark:opacity-5" />
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-display-xl font-bold text-[#2f211b] dark:text-white mb-6 animate-slide-up animate-delay-100">
              Welcome to <span className="text-primary-600 dark:text-primary-400">CWNU</span>
            </h1>
            <p className="text-body-lg text-[#5f5045] dark:text-gray-300 mb-8 max-w-2xl mx-auto animate-slide-up animate-delay-200">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-slide-up animate-delay-300">
              <Link to={primaryPath}>
                <Button size="lg" className="w-full sm:w-auto">
                  {!isAuthenticated ? 'Create student account' : user?.role === 'student' ? 'Explore courses' : 'Open dashboard'}
              ...
              <Link to="/about">
                <Button variant="outline" size="lg" className="w-full sm:w-auto border-primary-600 text-primary-700 hover:bg-primary-50 dark:border-white dark:text-white dark:hover:bg-white/10">
                  Learn about CWNU
                </Button>
              </Link>
            </div>

            <div className="mt-16 grid grid-cols-1 gap-6 text-center sm:grid-cols-3 sm:gap-8 animate-slide-up animate-delay-400">
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce">
          <svg className="w-6 h-6 text-[#9b8060] dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <section className="section relative overflow-hidden" aria-labelledby="cta-title">
        <div className="absolute inset-0 bg-gradient-to-r from-[#8f1d24] via-[#b42323] to-[#c8a951] dark:from-academic-navy dark:via-academic-navy-light dark:to-academic-dark" />
...
function StatItem({ value, label }: { value: string; label: string }) {
  return (
    <div className="text-center">
      <div className="text-4xl sm:text-5xl font-bold text-[#8f1d24] dark:text-white mb-1">{value}</div>
      <div className="text-body text-[#75685e] dark:text-gray-300">{label}</div>
    </div>
  );
}
